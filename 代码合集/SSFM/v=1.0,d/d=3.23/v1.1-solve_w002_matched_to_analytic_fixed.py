...
def solve_ssfm(
    ...
    data_path = out_dir / "v1.1_matched_slices_data.npz"
    np.savez_compressed(
        data_path,
        x=x,
        y=y,
    ...
    config_path = out_dir / "v1.1_matched_config.json"
    ...
    error_csv_path = out_dir / "v1.1_slice_errors.csv"
...
def plot_numerical_slices(result, p: Params) -> Path:
    ...
    fig.suptitle(
        rf"Numerical propagation: $w={p.w}$, "
        rf"$g_2={p.g2}$, $P(0)={p.target_power}$",
        fontsize=15,
    )

    path = out_dir / "v1.1_numerical_slices_matched_to_analytic.png"
...
def plot_comparison(result, p: Params) -> Path:
    ...
    fig.suptitle(
        rf"Matched coordinates: $\alpha=\beta_0 z$, "
        rf"$\beta_0={p.beta0:.3g}$, $P(0)={p.target_power}$",
        fontsize=15,
    )

    path = out_dir / "v1.1_analytic_vs_numerical_comparison.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path
...
def plot_power(result) -> Path:
    ...
    path = out_dir / "v1.1_power_drift.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path
