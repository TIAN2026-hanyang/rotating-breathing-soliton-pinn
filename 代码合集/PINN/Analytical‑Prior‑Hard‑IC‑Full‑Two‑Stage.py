...
def train_case(
    ...
    output_dir = SCRIPT_DIR / f"hard_ic_final_{tag}"
    ...
    total_history = np.maximum(
        np.asarray([h["total"] for h in history], dtype=np.float64),
        1.0e-30
    )

    analytic_history = np.maximum(
        np.asarray([h["analytic"] for h in history], dtype=np.float64),
        1.0e-30
    )
    ...
    order = [2, 1, 0, 3]  # total, analytic relative, PDE, PDE fine-tuning starts
