"""
classic_pinn_forward_w002_v1d323.py

Classical PINN baseline for the forward problem of the four-soliton molecule.

Purpose
-------
Use a standard PINN to solve the target PDE directly:

    i psi_z + psi_xx + psi_yy
    + 0.02 |psi|^2 psi - (x^2+y^2) psi = 0.

This is the baseline for comparing with the hard-IC analytical-prior PINN.
The network is classical:

    psi_theta(x,y,z) = N_theta(x,y,z),

with no analytical base embedding and no hard initial-condition lifting.

Kept consistent with the hard-IC code:
1. Same case: v1.0_d3.23 by default;
2. Same domain: x,y in [-8,8], z in [0,pi/2];
3. Same random seed;
4. Same PDE residual definition;
...
Difference from hard-IC PINN
----------------------------
Hard-IC PINN:
    psi_theta = psi_w0 + (z/zmax) * N_theta.

Classical PINN here:
    psi_theta = N_theta(x,y,z).

Training loss:
    L = lambda_ic * L_IC + lambda_bc * L_BC + lambda_pde * L_PDE.

The SSFM numerical solution is never used for training. It is used only after
...
class ClassicPINN(torch.nn.Module):
    """Classical PINN: directly maps (x,y,z) to (u,v)."""
...
def constituent_centroids_and_radius(
    ...
    Notes for classical PINN:
        If the learned field collapses to a near-zero solution, the local
        intensity can be too small to define a meaningful centroid. In that
        case, the code falls back to the analytical centre for numerical
        stability, and the physical failure should be interpreted from the
        field/intensity errors and power loss.
    """
...
def train_case(tag: str, cfg: Config, device: torch.device) -> dict:
    ...
    model = ClassicPINN(cfg, dtype).to(device)
    amplitude_scale = float(np.sqrt(np.max(np.abs(ic_psi) ** 2)))

    output_dir = SCRIPT_DIR / f"classic_soft_ic_pinn_nf{cfg.n_f}_{tag}"
    ...
    for step in range(1, total_adam_steps + 1):
        ...
        loss = cfg.lambda_ic * l_ic + cfg.lambda_bc * l_bc + cfg.lambda_pde * l_pde
        loss.backward()
    ...
    if cfg.lbfgs_steps > 0:
        ...
        def closure():
            ...
            loss = cfg.lambda_ic_lbfgs * l_ic + cfg.lambda_bc * l_bc + cfg.lambda_pde * l_pde
            loss.backward()
            return loss
    ...
    for i, z_value in enumerate(z_values):
        ...
        axes[0, i].set_title(rf"Classic PINN $z/\pi={z_value/np.pi:.1f}$")
    ...
    fig.suptitle(rf"Classical PINN: $\nu={nu}$, $d={d}$, $w=0.02$")
    ...
    summary = {
        ...
        "training_loss": "lambda_ic*L_IC + lambda_bc*L_BC + lambda_pde*L_PDE",
        "network_parameterization": "psi_theta = N_theta(x,y,z)",
        "sampling": {
...
def main() -> None:
    ...
    summaries = []
    print(f"device={device}; classical PINN baseline; w={cfg.w}; IC+BC+PDE")
    ...
    with (SCRIPT_DIR / "classic_pinn_run_summary.json").open("w", encoding="utf-8") as file:
