...
修正后的两阶段 PINN 正问题：
1. 硬初值嵌入，严格满足 psi(x,y,0)=psi0(x,y)；
2. 解析先验使用相对 L2 损失；
3. 解析先验点与 PDE 配点均采用“全域 LHS + 光束区域增强 LHS”混合抽样；
4. 第一阶段学习 w=0 解析先验；
5. 第二阶段使用 w=0.02 PDE，逐渐降低解析先验权重；
...
CASE_TAGS = ("v0.8_d4.00", "v1.0_d3.58", "v1.2_d3.27")
...
class Config:
    ...
    # Formula (36)(37): centroid trajectory and molecular radius diagnostics.
...
def relative_phases_numpy(cfg: Config) -> np.ndarray:
    n = np.arange(cfg.n_components, dtype=np.float64)
    phases = 2.0 * np.pi * cfg.phase_m * n / cfg.n_components
...
def normalization_factor(
    ...
    for n, phase0 in enumerate(relative_phases_numpy(cfg)):
        phi = 2.0 * np.pi * n / cfg.n_components
        x0 = d * np.cos(phi)
        y0 = d * np.sin(phi)
...
def beam_centres_numpy(
    ...
    alpha = beta0(cfg) * z
    phi = 2.0 * np.pi * beam_index / cfg.n_components
    x = d * (
...
def mixed_lhs_points(
    ...
    z = cfg.z_max * unit[:, 0]
    beam_index = np.minimum(
        (cfg.n_components * unit[:, 1]).astype(int),
        cfg.n_components - 1,
    )
...
class HardICPINN(torch.nn.Module):
    """
    Residual-transfer PINN.

    The w=0 analytic field is embedded as the base solution:
        psi_theta = psi_w0 + (z/zmax) * N_theta.

    Hence the initial condition is exact, and the neural network only learns
    the small nonlinear correction caused by w=0.02.
    """
    ...
    def analytic_uv(
        ...
        for n in range(self.cfg.n_components):
            phi = 2.0 * math.pi * n / self.cfg.n_components
            cp = math.cos(phi)
            sp = math.sin(phi)
    ...
    def forward(self, xyz: torch.Tensor) -> torch.Tensor:
        ...
        return analytic_base + lifting * self.correction(xyz)
...
def pde_evaluation_region_masks(
    ...
    for beam_index in range(cfg.n_components):
...
def constituent_centroids_and_radius(
    ...
    """
    Compute intensity-weighted constituent centroids and molecular radius.

    For each propagation slice z_k and each constituent beam n, the centroid is
    computed in a local region around the analytic beam centre. If the local
    intensity is too small, the analytic centre is used as a fallback.

    Returns
    -------
    centroids:
        Shape (n_z, n_components, 2), with last dimension (x_c, y_c).
    radius:
        Shape (n_z,), mean radial distance of constituent centroids from origin.
    """
    ...
    centroids = np.empty((n_z, n_components, 2), dtype=np.float64)
    radius = np.empty(n_z, dtype=np.float64)

    local_radius2 = float(cfg.centroid_region_radius) ** 2
    eps = float(cfg.centroid_eps)

    for k, z_value in enumerate(z_values):
        intensity = intensity_slices[k]
        ...
        for n in range(n_components):
            ...
            # Fallback: if the local disk is empty, use the whole Voronoi cell.
            if not np.any(mask):
                mask = nearest == n
...
def collective_motion_diagnostics(
    psi_prediction: np.ndarray,
    psi_reference: np.ndarray | None,
    x: np.ndarray,
    y: np.ndarray,
    z_values: np.ndarray,
    nu: float,
    ...
    """
    Formula (36)(37) diagnostics.

    Formula (36):
        normalized centroid trajectory error E_traj.

    Formula (37):
        molecular radius R(z), plus absolute/relative radius errors.
    """
    ...
    centroids_reference, radius_reference = constituent_centroids_and_radius(
        intensity_reference,
        x,
        y,
        z_values,
    ...
    trajectory_relative_error = float(
        np.sqrt(np.mean(np.sum(centroid_diff ** 2, axis=2))) / d
    )

    radius_absolute_errors = np.abs(radius_pinn - radius_reference)
    radius_relative_errors = radius_absolute_errors / (
        np.abs(radius_reference) + 1.0e-12
    )

    radius_error_normalized_by_d = float(
        np.sqrt(np.mean(radius_absolute_errors ** 2)) / d
    )

    return {
        "centroids_pinn": centroids_pinn,
        "radius_pinn": radius_pinn,
        "centroids_reference": centroids_reference,
        "radius_reference": radius_reference,
        "trajectory_relative_error": trajectory_relative_error,
        "radius_absolute_errors": radius_absolute_errors,
        "radius_relative_errors": radius_relative_errors,
        "radius_error_normalized_by_d": radius_error_normalized_by_d,
    }
...
def load_numerical_reference(tag: str):
    ...
    candidates = [
        REFERENCE_DIR / f"lhs_N6_{tag}.npz",
...
def train_case(
    ...
    output_dir = SCRIPT_DIR / f"N6_hard_ic_final_{tag}"
    ...
    for step in range(1, cfg.pretrain_steps + 1):
        ...
        loss = cfg.lambda_analytic_pre * l_ana + cfg.lambda_bc * l_bc
        loss.backward()
        optimizer.step()
    ...
    summary = {
        ...
        "hard_ic_relative_error": float(hard_ic_error),
    ...
    print(
        f"[{tag}] done: hard IC err={hard_ic_error:.3e}, "
...
#         "--case",
#         default="v1.0_d3.23",
#         choices=("all",) + CASE_TAGS,
#     )
#     parser.add_argument("--hidden-layers", type=int, default=6)
#     parser.add_argument("--width", type=int, default=80)
#     parser.add_argument("--n-analytic", type=int, default=2500)
#     parser.add_argument("--nf", type=int, default=20000)
#     parser.add_argument("--pretrain-steps", type=int, default=3000)
#     parser.add_argument("--finetune-steps", type=int, default=7000)
#     parser.add_argument("--lbfgs-steps", type=int, default=0)
#     parser.add_argument("--analytic-batch", type=int, default=256)
#     parser.add_argument("--boundary-batch", type=int, default=256)
#     parser.add_argument("--pde-batch", type=int, default=256)
#     parser.add_argument("--pde-eval-points", type=int, default=10000)
#     parser.add_argument("--pde-eval-batch", type=int, default=512)
#     parser.add_argument("--beam-region-radius", type=float, default=2.0)
#     parser.add_argument("--boundary-region-width", type=float, default=1.0)
#     parser.add_argument("--lambda-analytic-end", type=float, default=0.05)
...
#         default="zero",
#     )
#     parser.add_argument("--prediction-grid", type=int, default=128)
#     parser.add_argument(
#         "--dtype",
...
#         width=args.width,
#         n_analytic=args.n_analytic,
#         n_f=args.nf,
#         pretrain_steps=args.pretrain_steps,
#         finetune_steps=args.finetune_steps,
#         lbfgs_steps=args.lbfgs_steps,
#         analytic_batch=args.analytic_batch,
#         boundary_batch=args.boundary_batch,
#         pde_batch=args.pde_batch,
#         pde_eval_points=args.pde_eval_points,
#         pde_eval_batch=args.pde_eval_batch,
#         beam_region_radius=args.beam_region_radius,
#         boundary_region_width=args.boundary_region_width,
#         lambda_analytic_end=args.lambda_analytic_end,
...
#     )
#     tags = CASE_TAGS if args.case == "all" else (args.case,)
#     summaries = []
#     print(f"device={device}; hard IC + relative loss + beam-enhanced LHS")
#     for tag in tags:
