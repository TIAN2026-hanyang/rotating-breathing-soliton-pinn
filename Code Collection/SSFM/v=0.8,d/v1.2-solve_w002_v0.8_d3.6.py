...
def solve_ssfm(
    ...
    data_path = out_dir / "v0.8_d3.6_matched_slices_data.npz"
    np.savez_compressed(
        data_path,
        x=x,
        y=y,
    ...
    config_path = out_dir / "v0.8_d3.6_matched_config.json"
    ...
    error_csv_path = out_dir / "v0.8_d3.6_slice_errors.csv"
...
def plot_numerical_slices(result, p: Params) -> Path:
    ...
    path = out_dir / "v0.8_d3.6_numerical_slices.png"
...
def plot_comparison(result, p: Params) -> Path:
    ...
    path = out_dir / "v0.8_d3.6_analytic_vs_numerical.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path
...
def plot_power(result) -> Path:
    ...
    path = out_dir / "v0.8_d3.6_power_drift.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path
