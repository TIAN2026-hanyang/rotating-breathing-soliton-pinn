"""
N=6 SSFM reference generator for the analytical-prior PINN project.

Solves
    i psi_z + psi_xx + psi_yy + w |psi|^2 psi + g2 (x^2+y^2) psi = 0
with w=0.02, g2=-1.

The script generates three N=6 reference files compatible with the PINN loader:
    lhs_N6_v0.8_d4.00.npz
    lhs_N6_v1.0_d3.58.npz
    lhs_N6_v1.2_d3.27.npz

For N=6 and alternating phases theta_n=n*pi, use m=3 because
    theta_n = 2*pi*m*n/N = n*pi.
"""
...
class Params:
    ...
    # N=6 molecule and m=3 gives theta_n = n*pi.
    n_components: int = 6
    phase_m: int = 3
    ...
    def relative_phases(self) -> np.ndarray:
        n = np.arange(self.n_components, dtype=np.float64)
        phases = 2.0 * np.pi * self.phase_m * n / self.n_components
    ...
    def validate(self) -> None:
        ...
        if self.phase_m < 0:
            raise ValueError("phase_m 必须为非负整数。")
...
class Numerics:
    ...
    # 这里存的是 alpha，不是 z。g2=-1 时 beta0=2，所以 z=alpha/2。
...
def raw_initial_field(X: np.ndarray, Y: np.ndarray, p: Params) -> np.ndarray:
    """w=0 N-component analytic field at z=0 before power normalization."""
    beta0 = p.beta0
    phases = p.relative_phases
    psi0 = np.zeros_like(X, dtype=np.complex128)

    for n in range(p.n_components):
        varphi = 2.0 * np.pi * n / p.n_components
        x0 = p.d * np.cos(varphi)
        y0 = p.d * np.sin(varphi)

        # Initial transverse momentum p_n(0)=R'_n(0)/2.
        px0 = -0.5 * beta0 * p.nu * p.d * np.sin(varphi)
        py0 = 0.5 * beta0 * p.nu * p.d * np.cos(varphi)

        q2 = (X - x0) ** 2 + (Y - y0) ** 2
        phase = px0 * X + py0 * Y + phases[n]

        psi0 += p.A0 * np.exp(-q2 / (2.0 * p.a0**2) + 1j * phase)

    return psi0


def discrete_power(psi: np.ndarray, dx: float, dy: float) -> float:
    """2D rectangular quadrature on the periodic FFT grid."""
...
def analytic_w0_field(
    ...
    """Exact w=0 N-component analytical field."""
    beta0 = p.beta0
    alpha = beta0 * z
    ca = np.cos(alpha)
    sa = np.sin(alpha)
    ...
    # Quadratic chirp; zero when Pr=1.
    b = beta0 * (p.Pr - 1.0) * np.sin(2.0 * alpha) / (8.0 * D)

    theta = -np.arctan2(np.sqrt(p.Pr) * sa, ca)
    common_phase = beta0 * p.d**2 * (1.0 - p.nu**2) * np.sin(2.0 * alpha) / 8.0
    ...
    for n in range(p.n_components):
        ...
        pxn = -0.5 * beta0 * p.d * (cphi * sa + p.nu * sphi * ca)
        pyn = 0.5 * beta0 * p.d * (-sphi * sa + p.nu * cphi * ca)

        q2 = (X - xn) ** 2 + (Y - yn) ** 2
        phase = b * q2 + pxn * X + pyn * Y + common_phase + theta + phases[n]
...
def solve_ssfm(p: Params, n: Numerics, tag: str):
    p.validate()
    n.validate()
    ...
    psi, G0, raw_power = normalized_initial_field(X, Y, dx, dy, p)
    ...
    for step in range(1, n.n_steps + 1):
        ...
        if next_slice < len(target_steps) and step == target_steps[next_slice]:
            numerical_slices.append(psi.copy())
            analytic_slices.append(analytic_w0_field(X, Y, z_now, p, G0))
    ...
    field_errors = np.array(
        [relative_l2(numerical_slices[j], analytic_slices[j]) for j in range(len(actual_z))]
    )
    intensity_errors = np.array(
        [relative_l2(intensity_num[j], intensity_ana[j]) for j in range(len(actual_z))]
    )
    boundary_max = np.array([boundary_max_intensity(s) for s in numerical_slices])

    data_path = out_dir / f"lhs_{tag}.npz"
    np.savez_compressed(
        data_path,
        # Names expected by the current PINN loader.
        grid_x=x,
        grid_y=y,
        grid_z=actual_z,
        psi_slices=numerical_slices,
        ...
        n_components=np.array(p.n_components),
        phase_m=np.array(p.phase_m),
    )

    config_path = out_dir / f"ssfm_config_{tag}.json"
    with config_path.open("w", encoding="utf-8") as f:
        config = {
            "tag": tag,
            "physics": asdict(p),
            "relative_phases": p.relative_phases.tolist(),
            "numerics": asdict(n),
    ...
    error_csv_path = out_dir / f"ssfm_slice_errors_{tag}.csv"
    with error_csv_path.open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writerow(
            [
                ...
                "z_over_pi",
                "z",
                "relative_field_error_vs_w0_analytic",
                "relative_intensity_error_vs_w0_analytic",
...
def plot_numerical_slices(result, p: Params) -> Path:
    ...
    fig, axes = plt.subplots(1, len(z_values), figsize=(26, 4.6), constrained_layout=True)

    last = None
    for j, ax in enumerate(np.atleast_1d(axes)):
        last = draw_intensity(ax, x, y, intensity[j], 0.0, vmax)
        ax.set_title(rf"$z/\pi={z_values[j]/np.pi:.1f}$")
        ax.set_xlabel(r"$x$")
        ax.set_ylabel(r"$y$")

    fig.colorbar(last, ax=np.atleast_1d(axes).tolist(), shrink=0.84, label=r"$|\psi_{\rm SSFM}|^2$")
    fig.suptitle(rf"N=6 SSFM: $\nu={p.nu}$, $d={p.d}$, $w={p.w}$", fontsize=15)

    path = out_dir / f"ssfm_numerical_slices_{tag}.png"
...
def plot_comparison(result, p: Params) -> Path:
    ...
    fig, axes = plt.subplots(3, len(z_values), figsize=(25, 11), constrained_layout=True)
    im_main = None
    im_diff = None

    for j in range(len(z_values)):
        im_main = draw_intensity(axes[0, j], x, y, I_ana[j], 0.0, vmax)
        draw_intensity(axes[1, j], x, y, I_num[j], 0.0, vmax)
        im_diff = draw_intensity(axes[2, j], x, y, diff[j], 0.0, diff_vmax)
        axes[0, j].set_title(rf"$z/\pi={z_values[j]/np.pi:.1f}$")
        for row in range(3):
            axes[row, j].set_xlabel(r"$x$")
            axes[row, j].set_ylabel(r"$y$")

    axes[0, 0].set_ylabel("Analytic $w=0$\n" + r"$y$")
    axes[1, 0].set_ylabel("SSFM $w=0.02$\n" + r"$y$")
    axes[2, 0].set_ylabel("Absolute intensity difference\n" + r"$y$")

    fig.colorbar(im_main, ax=axes[:2, :].ravel().tolist(), shrink=0.75, label="Intensity")
    fig.colorbar(im_diff, ax=axes[2, :].ravel().tolist(), shrink=0.75, label=r"$|I_{\rm SSFM}-I_{w=0}|$")
    fig.suptitle(rf"N=6 matched coordinates: $\alpha=\beta_0 z$, $\beta_0={p.beta0:.3g}$", fontsize=15)

    path = out_dir / f"ssfm_analytic_vs_numerical_{tag}.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path
...
def plot_power(result) -> Path:
    ...
    ax.set_title(f"Relative power drift: {tag}")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()

    path = out_dir / f"ssfm_power_drift_{tag}.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path
...
def main() -> None:
    cases = {
        "N6_v0.8_d4.00": (0.8, 4.00),
        "N6_v1.0_d3.58": (1.0, 3.58),
        "N6_v1.2_d3.27": (1.2, 3.27),
    }
    ...
    summaries = []
    for tag, (nu, d) in cases.items():
        p = Params(nu=nu, d=d, n_components=6, phase_m=3)
        result = solve_ssfm(p, n, tag)
        numerical_path = plot_numerical_slices(result, p)
        comparison_path = plot_comparison(result, p)
        power_path = plot_power(result)

        final_power_drift = (result["power"][-1] - result["power"][0]) / result["power"][0]
        max_power_drift = np.max(np.abs(result["power"] - result["power"][0])) / result["power"][0]
        ...
        summary = {
            ...
            "raw_power": result["raw_power"],
            "G0": result["G0"],
            "normalized_P0": result["power"][0],
            "initial_field_mismatch": result["initial_field_mismatch"],
            "final_relative_power_drift": float(final_power_drift),
            "max_relative_power_drift": float(max_power_drift),
            "alpha_over_pi_slices": (result["alpha"] / np.pi).tolist(),
            "z_over_pi_slices": (result["z"] / np.pi).tolist(),
            "relative_intensity_errors_vs_w0_analytic": result["intensity_errors"].tolist(),
            "data_path": str(result["data_path"]),
            "config_path": str(result["config_path"]),
            "error_csv_path": str(result["error_csv_path"]),
            "numerical_figure": str(numerical_path),
            "comparison_figure": str(comparison_path),
            "power_figure": str(power_path),
        }
        summaries.append(summary)

        print("=" * 80)
        print(f"N=6 SSFM finished: {tag}")
        print(f"nu, d                       = {nu}, {d}")
        print(f"phases                      = {p.relative_phases}")
        print(f"beta0                       = {p.beta0:.12f}")
        print(f"z/pi slices                 = {result['z'] / np.pi}")
        ...
        print(f"initial field mismatch      = {result['initial_field_mismatch']:.6e}")
        print(f"final relative power drift  = {final_power_drift:.6e}")
        print(f"max relative power drift    = {max_power_drift:.6e}")
        print(f"data file                   = {result['data_path']}")

    summary_path = SCRIPT_DIR / "ssfm_N6_three_cases_summary.json"
    with summary_path.open("w", encoding="utf-8") as f:
        json.dump(summaries, f, ensure_ascii=False, indent=2)

    print("=" * 80)
    print(f"All N=6 SSFM references finished. Summary: {summary_path}")
